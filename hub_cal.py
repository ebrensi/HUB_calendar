...
"""
  We assume that initial data is in the form of a CSV file output by something like gcal2excel,
with fields 'Title', 'Start', 'End', 'Where', and 'Calendar'
"""

# 3/10/2014 is the date that Hub staff started using consistent labling of events
Event_Location_Label = 'Loc' # This is the name for the room label classification 
...
def parse_calendar(infile,outfile, start_date='', end_date='', calendar=''):
    ...
    df = df.drop_duplicates(subset=['Title','Start','End']) # drop all duplicates of Title, Start and End field
    
    df = df.sort('Start').reset_index(drop=True)  # sort by start time and reindex
    ...
    # These are the exact uppper case labels associated with most events on hub calendar in the 'Title' field. 
    # First we try to match these exactly
    labels = {'GALLERY' : ['OMI GALLERY','GALLERY','\bOMI\b'],
              'ATRIUM' : ['ATRIUM'],
              'BROADWAY' :['ON BROADWAY','BROADWAY ROOM','MAIN STAGE','MAIN FLOOR','MAIN SPACE','BROADWAY'],
              ...
              'CLOSED' : ['CLOSED FOR RENTAL', 'CLOSED','(?:de)?[ -]?install(?:ation)?','Booked due to','event use',
                          'cancell?ed','not available','event prep','storage','Co-?Working','set ?up'],
              'HOLD' : ['HOLD']
              }
    ...
    where_exclude_text = ["Impact Hub Oakland", "2323", "Broadway", "Oakland, CA", "94612", "United States",",",
    ...
    for sup_label in labels.keys():
        ...
        for label in labels[sup_label]:
            ...
            text_contains_label = text.str.contains(label, case=False, na=False)
            ...
            if any(text_contains_label):
                ...
                print
                text.loc[text_contains_label] = text.loc[text_contains_label].str.replace(label,'',case=False)
            print('%d records contain %s in Title/Where') % (text_contains_label.sum(), label)
        print('***** %d records contain %s in Title/Where *****') % (df[sup_label].sum(), sup_label)
    
    
    # Check Description field for cancellations    
    Description_contains_cancelled = df.Description.str.contains('cancelled',na=False,case=False)
...
## ****************************************************************************    
"""
#parse_calendar('HUB_cal_All.csv','classified_loc.csv',start_date='2/2014',calendar='Conference Room')
...
rate_lookup = pd.DataFrame({
                ...
                'MEDITATION':[0,0,0,0],
                'KITCHEN':[0,0,0,0]     }, index=['nonmember','parttime','fulltime','free'])

# For title search
org_status = {'set[ -]?up|konda|km|simka|ashara|Dakara?i|maiki|Mikayla|zakiya|Lisa|calgary|staff|interview|HUB Oak|founders':'free', #this matches hub internal meetings
...
print('\nDetermining rates/income for %d records...') % n

# Extract records indicating no fee in description
free = T.Description.str.contains('gratis|no.charge|\$0|no cost|iho[^m]*me?e?ti?n?g', case=False, na=False)
df.loc[free[free].index,'Rate'] = 0
T = T.loc[~free]
print('...%d records explicitly labeled "no-charge" in Description') % free.sum() 


# Extract rates by matching $x/hr, $x/hour, $x per hour, etc in description
rates1 = T.Description.str.extract('\$(\d+).{0,3}(?:/|per| ) ?ho?u?r',flags=re.IGNORECASE).dropna().astype(float)
df.loc[rates1.index, 'Rate'] = rates1 
print('...%d records descriptions explicitly labeled "()/hr" or "()/hour"') % rates1.count()
#print rates1
T = T.drop(rates1.index)

rates2 = T.Description.str.extract('\$(\d+) x ',flags=re.IGNORECASE).dropna().astype(float)
df.loc[rates2.index, 'Rate'] = rates2
print('...%d records descriptions explicitly labeled "$() x "') % rates2.count()
#print rates2
T = T.drop(rates2.index)

rates3 = T.Description.str.extract(' x \$(\d+)',flags=re.IGNORECASE).dropna().astype(float)
df.loc[rates3.index, 'Rate'] = rates3
print('...%d records descriptions explicitly labeled " x $()"') % rates3.count()
...
for kw in desc_keywords.keys():
    ...
    print('\n...%d descriptions containing "%s", which has "%s" status') %(rates.count(), kw, desc_keywords[kw] )
...
for org in org_status.keys():
    ...
    df.loc[rates.index, 'Rate'] = rates 
    print('\n...%d titles containing "%s", which has "%s" status') %(rates.count(),org,org_status[org] )
    print df.loc[rates.index, ['Title','Rate']]
    T = T.drop(rates.index)
...
print('\n...%d Titles contain "future sound" which gets a rate of $18') % tfs.count()
T = T.drop(tfs.index)
...
total = T.Description.str.extract('(?:total|charging|charge|price|paid)[^$]*\$(\d+)', flags=re.IGNORECASE).dropna()
df.loc[total.index,'Charge'] = total
T = T.drop(total.index)
print('\n...%d Descriptions contain explicit total charge') % total.count()

# Whatever record descriptions have $ in them we will go ahead and call those total amounts
total2 = T.Description.str.extract('\$(\d+)', flags=re.IGNORECASE).dropna()
df.loc[total2.index,'Charge'] = total2
print('\n...%d Descriptions contain implicit total charge') % total2.count()
print total2
T = T.drop(total2.index)

#totals_without_rates = df.Charge.notnull() & df.Rate.isnull()
#df.Rate[totals_without_rates] = df.Charge[totals_without_rates] / df.Duration[totals_without_rates]
...
df2[['Start','Loc','Duration','Rate','Charge','Title','Description']].to_csv('classified_loc_charge.csv')
"""

df2 = import_parsed_csv('classified_loc_charge.csv')
bk = df2[['Start','Rate','Duration','Charge',Event_Location_Label]].dropna(subset=[Event_Location_Label,'Charge'])
...
pv = pd.pivot_table(bk, columns=Event_Location_Label,index='Start',values='Duration', aggfunc=pd.np.sum)
...
pv = pd.pivot_table(bk, columns=Event_Location_Label, index='Start',values='Charge', aggfunc=pd.np.sum)
income_by_month = pv[conf_rooms].fillna(0).resample('M',how='sum')
ax = income_by_month.plot(figsize=(15,5),title='\$ Dollars \$ over '+ drange)
income_by_month.plot(style='o', ax=ax, legend=False)
#print income_by_month



pv = pd.pivot_table(bk, index='Rate', columns='Loc', values='Duration',aggfunc=pd.np.sum)[conf_rooms]
pv_adj = pv / pv.sum()

for room in conf_rooms:
    plt.figure()    
    pv_adj[room].dropna().plot(kind='bar',title=room +': Rate distribution over '+drange ,figsize=(12,5))


pv = pd.pivot_table(bk, index='Rate', columns='Loc', values='Charge',aggfunc=pd.np.sum)[conf_rooms]
plt.figure()    
...
pv = pd.pivot_table(bk, index='Rate', columns='Loc', values='Duration',aggfunc=pd.np.sum)[conf_rooms]
plt.figure()    
pv.sum().plot(kind='bar',title='Total rented hours '+drange,figsize=(12,5))


pv = pd.pivot_table(bk, index='Rate', columns='Loc', values='Duration',aggfunc=lambda x: x.count())[conf_rooms]
plt.figure()    
pv.sum().plot(kind='bar',title='Total times rented '+drange,figsize=(12,5))
